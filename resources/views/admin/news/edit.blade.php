										...
										<div id="basic-one" class="collapse show ctm-padding" aria-labelledby="basic1" data-parent="#accordion-details">
											...
											<form method="POST" action="{{ route('news.update', $news->id) }}">
												...
												<div class="row">
													...
													<div class="col-sm-3">
														<div class="form-group">
															<input type="text" class="form-control {{ $errors->has('title') ? 'is-invalid' : ''}}" name="title" value="{{ old('title', $news->title) }}" maxlength="44" autocomplete="off">
													...
													<div class="col-sm-3">
														<div class="form-group">
															<select class="form-control select {{ $errors->has('category') ? 'is-invalid' : ''}}" name="category" id="category" style="width: 100%">
																...
																<option value="Information" {{ old ('category', $news->category) == 'Information' ? 'selected' : ''}}>Information</option>
																<option value="Important" {{ old ('category', $news->category) == 'Important' ? 'selected' : ''}}>Important</option>
																<option value="Event" {{ old ('category', $news->category) == 'Event' ? 'selected' : ''}}>Event</option>
																<option value="Project" {{ old ('category', $news->category) == 'Project' ? 'selected' : ''}}>Project</option>
															</select>
													...
													<div class="col-sm-3">
														<div class="form-group">
															<select class="form-control select" name="status">
															    ...
															    <option value='In active' {{ old ('status', $news->status) == "In active" ? 'selected' : '' }}>In-active</option>
															</select>
